export default function SoftKey3W() {
  ...
  if (u.substring(0, 5) == "https") {
    if (isIE11 || isEDGE) {
      ...
      url = "wss://127.0.0.1:8888/websocket";
    } else {
  ...
  if (window.WebSocket) {
    // this.Socket_UK = new WebSocket("ws://localhost:8888/websocket","usbkey-protocol");
    this.Socket_UK = new WebSocket("ws://127.0.0.1:4006/xxx","usbkey-protocol");
    // this.Socket_UK = new WebSocket("ws://localhost:8888/websocket",);
    /*this.Socket_UK.onmessage = function (event) {
      console.log("onmessage")
      // var ta = document.getElementById('responseText');
      // ta.value = ta.value + '\n' + event.data
    };
    this.Socket_UK.onopen = function (event) {
      console.log("onopen-->连接开启!")
      // var ta = document.getElementById('responseText');
      // ta.value = "连接开启!";
    };
    this.Socket_UK.onclose = function (event) {
      console.log("onclose-->连接被关闭")
      // var ta = document.getElementById('responseText');
